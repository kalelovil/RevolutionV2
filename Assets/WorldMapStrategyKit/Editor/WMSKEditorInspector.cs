...
namespace WorldMapStrategyKit {
	...
	public partial class WMSKEditorInspector : Editor {
		...
		void OnEnable () {
			...
			// Load UI icons
			Texture2D[] icons = new Texture2D[21];
			...
			// Setup main toolbar
			mainToolbarIcons = new GUIContent[6];
			...
			mainToolbarIcons [3] = new GUIContent ("Map Generator", icons [20], "Randomly generates a map");
			mainToolbarIcons [4] = new GUIContent ("Revert", icons [2], "Restore shape information");
			mainToolbarIcons [5] = new GUIContent ("Save", icons [3], "Confirm changes and save to file");
		...
		public override void OnInspectorGUI () {
			...
			try {
				...
				_editor.operationMode = (OPERATION_MODE)GUILayout.SelectionGrid ((int)_editor.operationMode, mainToolbarIcons, 3, toolbarStyle, GUILayout.Height (48));
		...
		void ShowEntitySelectors () {
			...
			if (_editor.countryIndex >= 0) {
				...
				GUILayout.Label  (_editor.countryIndex.ToString());
				EditorGUILayout.EndHorizontal ();
				EditorGUILayout.BeginHorizontal ();
			...
			if (_editor.editingMode == EDITING_MODE.PROVINCES) {
				...
				if (selectedProvincesCount > 1) {
					...
					int poolCountryIndex = _map.GetCountryIndex ("Pool");
					...
					if (_editor.provinceIndex >= 0) {
						EditorGUILayout.BeginHorizontal ();
						GUILayout.Label  ("   Id", GUILayout.Width(90));
						GUILayout.Label  (_editor.provinceIndex.ToString());
						EditorGUILayout.EndHorizontal ();
						EditorGUILayout.BeginHorizontal ();
						GUILayout.Label ("   Name", GUILayout.Width (90));
						...
						EditorGUILayout.BeginHorizontal ();
						GUILayout.Label (" ", GUILayout.Width (90));
						if (GUILayout.Button ("Simplify/Reduce Provinces", GUILayout.Width (180))) {
			...
			if (_editor.countryIndex >= 0) {
				...
				if (_editor.cityIndex >= 0) {
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label  ("   Id", GUILayout.Width(90));
					GUILayout.Label  (_editor.cityIndex.ToString());
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Name", GUILayout.Width (90));
		...
		void ShowReshapingRegionTools () {
			...
			if (_editor.reshapeRegionMode.hasCircle ()) {
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Circle Width", GUILayout.Width (120));
			...
			if (_editor.reshapeRegionMode == RESHAPE_REGION_TOOL.POINT || _editor.reshapeRegionMode == RESHAPE_REGION_TOOL.CIRCLE || _editor.reshapeRegionMode == RESHAPE_REGION_TOOL.ERASER) {
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Selected Region Only", GUILayout.Width (120));
				_editor.circleCurrentRegionOnly = EditorGUILayout.Toggle (_editor.circleCurrentRegionOnly, GUILayout.Width (20));
				EditorGUILayout.EndHorizontal ();

				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Cursor", GUILayout.Width (120));
				GUILayout.Label ("X: " + _editor.cursor.x.ToString ("F7") + ", Y: " + _editor.cursor.y.ToString ("F7"));
				EditorGUILayout.EndHorizontal ();

				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Snap Precision Digits:", GUILayout.Width (120));
				string snapUnitStr = _editor.snapPrecisionDigits.ToString ();
				string newSnapUnit = GUILayout.TextField (snapUnitStr, GUILayout.Width (90));
			...
			case RESHAPE_REGION_TOOL.CIRCLE:
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Constant Move", GUILayout.Width (120));
			...
			case RESHAPE_REGION_TOOL.MAGNET:
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Agressive Mode", GUILayout.Width (120));
				_editor.magnetAgressiveMode = EditorGUILayout.Toggle (_editor.magnetAgressiveMode, GUILayout.Width (20));
				EditorGUILayout.EndHorizontal ();
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Include Countries", GUILayout.Width (120));
				_editor.magnetIncludeCountries = EditorGUILayout.Toggle (_editor.magnetIncludeCountries, GUILayout.Width (20));
				EditorGUILayout.EndHorizontal ();
				break;
		...
		static void ExportProvincesMap (MenuCommand command) {
		...
		[MenuItem ("CONTEXT/WMSK_Editor/Update Water Motion Vectors", false, 180)]
		static void UpdateWaterMotionVectorsMenuOption (MenuCommand command) {
			WMSK_Editor editor = (WMSK_Editor)command.context;
			...
			Texture2D terretrialTex = WMSK.instance.waterMask;
		...
		[MenuItem ("CONTEXT/WMSK_Editor/Equalize Provinces", false, 132)]
		...
		void ShowCandidatePoint () {
			...
			for (int p = 0; p < max; p++) {
				int q = p == 0 ? max - 1 : p - 1;
//																Vector2 rp = (region.points [p] + region.points [q]) * 0.5f;
