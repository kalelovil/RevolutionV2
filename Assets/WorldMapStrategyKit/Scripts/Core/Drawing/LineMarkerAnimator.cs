...
namespace WorldMapStrategyKit {
	...
	public class LineMarkerAnimator : MonoBehaviour {
		...
		Vector3 startCapPos0, startCapPos1;
		Vector3 endCapPos0, endCapPos1;
		Vector3 lastDashVertex0, lastDashVertex1;
		...
		void UpdateLine () {
			...
			if (useArrowStartCap) {
				UpdateArrowCap (ref startCapPlaceholder, ref startCapGO, startCap, startCapFlipDirection, startCapPos0, startCapPos1, startCapScale, startCapOffset, startCapMaterial);
			}
			if (useArrowEndCap && endCapPos0 != Misc.Vector3zero) {
				UpdateArrowCap (ref endCapPlaceholder, ref endCapGO, endCap, endCapFlipDirection, endCapPos1, endCapPos0, endCapScale, endCapOffset, endCapMaterial);
			}
		}

		void UpdateArrowCap (ref GameObject placeholder, ref GameObject obj, GameObject cap, bool flipX, Vector3 startPos, Vector3 endPos, Vector3 scale, float offset, Material capMaterial) {
			if (obj == null) {
				...
				if (spr != null) {
					spr.color = lineMaterial.color;
				}
				MeshRenderer mr = obj.GetComponent<MeshRenderer> ();
				if (mr != null) {
					mr.sharedMaterial = capMaterial != null ? capMaterial : lineMaterial;
				}
			...
			if (flipX) {
				FastVector.Swap (ref startPos, ref endPos);
			}

			// set position
			Vector3 pos = Misc.Vector3zero;
			if (!usesViewport) {
				startPos = map.transform.TransformPoint (startPos);
				endPos = map.transform.TransformPoint (endPos);
			}

			// Length of cap based on size
			if (startPos != endPos) {
				Vector3 dir = Misc.Vector3zero;
				FastVector.NormalizedDirection (ref startPos, ref endPos, ref dir);
				pos = startPos + dir * offset;
				if (!obj.activeSelf) {
					obj.SetActive (true);
				}
				...
				if (usesViewport) {
					...
					obj.transform.LookAt (endPos + dir * 100f, camDir);
				} else {
		...
		void UpdateContinousLine (float t) {
			...
			if (currentVertex >= 0 && currentVertex < vertexCount) {
				...
				Vector3 progress = Vector3.Lerp (currentVertexPos, nextVertexPos, subt);
				lr.SetPosition (currentVertex, progress);

				// set line cap positions
				startCapPos0 = vertices [0];
				endCapPos0 = progress;
				if (vertexCount > 1) {
					startCapPos1 = vertices [1];
					endCapPos1 = nextVertexPos;
				} else {
					startCapPos1 = startCapPos0;
					endCapPos1 = endCapPos0;
				}
			}
		}
