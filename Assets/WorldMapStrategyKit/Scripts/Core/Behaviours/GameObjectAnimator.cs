	...
	public class GameObjectAnimator : MonoBehaviour, IExtendableAttribute {
		...
		public bool isOnWater {
			get { 
				...
				if (onWaterPosition != _currentMap2DLocation) {
		...
		/// </summary>
		public List<int> GetCellNeighbours () {
			int cellIndex = map.GetCellIndex (_currentMap2DLocation);
			if (cellIndex < 0)
				return null;
			return map.GetCellNeighbours (cellIndex, maxSearchCost + 1, maxSearchCost, terrainCapability);
		}
		...
		public void UpdateTransformAndVisibility (bool forceUpdateRotation) {
			...
			if (autoScale && !renderViewportIsTerrain) {
				if (map.renderViewportIsEnabled) {
					...
					float minScale = map.renderViewportGOAutoScaleMin;
					float maxScale = map.renderViewportGOAutoScaleMax;
					if (desiredScale < minScale) {
						desiredScale = minScale;
					} else if (desiredScale > maxScale) {
						desiredScale = maxScale;
					}
					transform.localScale = this.originalScale * desiredScale;
			...
			Vector3 worldPos = map.Map2DToWorldPosition (_currentMap2DLocation, h, transform.localScale.y * pivotY, heightMode, false);
			...
			if (isVisibleInViewport || renderViewportIsTerrain) {
				...
				if (isSprite || (!autoRotation && !preserveOriginalRotation)) {
					...
					bool hasNormal;
					if (map.renderViewportIsEnabled) {
						hasNormal = map.wrapHorizontally ? map.RenderViewportGetNormal (_currentMap2DLocation, out normal) : map.RenderViewportGetNormal (worldPos, out normal);
					} else {
						hasNormal = true;
		...
		public void UpdateVisibility (bool forceRefreshVisibility) {
			...
			if (map.renderViewportIsTerrain) {
				...
				shouldBeVisible = shouldBeVisible && (!map.renderViewportIsEnabled || (gameObject.activeSelf && map.renderViewportRect.Contains (map.Map2DToRenderViewport (_currentMap2DLocation))));
			}
		...
		void CheckBuoyancyEffect () {
			if (enableBuoyancyEffect && map.VGOBuoyancyAmplitude > 0 && map.lastKnownZoomLevel <= map.VGOBuoyancyMaxZoomLevel && isOnWater) {
				// Buoyancy effect
