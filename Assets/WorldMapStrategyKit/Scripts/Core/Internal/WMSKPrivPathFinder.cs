...
namespace WorldMapStrategyKit {
	...
	public partial class WMSK : MonoBehaviour {
		...
		void ComputeRouteMatrix(TERRAIN_CAPABILITY terrainCapability, float minAltitude, float maxAltitude) {
			...
			if (finder == null) {
				if (_customRouteMatrix == null || !pathFindingEnableCustomRouteMatrix)
					PathFindingCustomRouteMatrixReset();
		...
		void ComputeCellsCostsInfo() {
			...
			for (int k = 0; k < cellsCount; k++) {
				...
				if (usesViewport) {
					px = (int)(x * EARTH_ELEVATION_WIDTH);
					py = (int)(y * EARTH_ELEVATION_HEIGHT);
					float elev = viewportElevationPoints[py * EARTH_ELEVATION_WIDTH + px];
		...
		int FindRouteCountryValidator(int countryIndex) {
		...
		/// </summary>
		/// <returns>The extra cross cost.</returns>
		int FindRouteProvinceValidator(int provinceIndex) {
		...
		/// </summary>
		/// <returns>The extra cross cost.</returns>
		int FindRouteCellValidator(int cellIndex) {
		...
		void UpdatePathfindingMatrixCostTexture() {
			...
			for (int k = 0; k < colors.Length; k++) {
				if (_customRouteMatrix[k] < 0) {
					...
					float t = Mathf.Clamp01(_customRouteMatrix[k] / maxValue);
					colors[k] = new Color(1f, t, t);
				}
			}
