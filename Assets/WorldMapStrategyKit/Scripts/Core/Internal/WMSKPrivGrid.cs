...
namespace WorldMapStrategyKit {
	...
	public partial class WMSK : MonoBehaviour {
		...
		Vector3[] hexPoints = new Vector3[9];
		...
		// Caches
		Dictionary<int, Renderer> gridSurfaces;
		...
		void DestroyGridSurfaces () {
			...
			if (gridSurfaces != null) {
				...
				for (int k = 0; k < cached.Count; k++)
					if (cached [k] != null)
						DestroyImmediate (cached [k].gameObject);
				gridSurfaces.Clear ();
			}
			if (_gridSurfacesLayer != null)
		...
		public void DrawGrid () {
			...
			if (gridSurfaces == null) {
				gridSurfaces = new Dictionary<int, Renderer> ();
			} else {
				gridSurfaces.Clear ();
			}
			...
			if (cellHighlightedObjRenderer != null) {
				if (!cellHighlighted.isFading) {
					if (cellHighlighted.customMaterial != null) {
						cellHighlightedObjRenderer.sharedMaterial = cellHighlighted.customMaterial; // ApplyMaterialToSurface (cellHighlightedObj, cellHighlighted.customMaterial);
					} else {
