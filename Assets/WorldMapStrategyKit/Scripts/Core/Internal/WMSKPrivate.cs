	...
	public partial class WMSK : MonoBehaviour
	{
		...
		void PerformUserInteraction ()
		{
				...
				if (wheelAccel != 0) {
					...
					if (wheelAccel >= 0.01f || wheelAccel <= -0.01f) {
						...
						if (_currentCamera.orthographic) {
							...
							_currentCamera.transform.Translate ((_currentCamera.transform.position - dest) * wheelAccel * _mouseWheelSensitivity, Space.World);
						}
						wheelAccel /= 1.15f;
					} else {
						wheelAccel = 0;
						...
						if (_allowUserDrag || _allowUserKeys) {
							if (_dragConstantSpeed) {
								...
								if (dragDirection.x != 0 || dragDirection.y != 0) {
									...
									_currentCamera.transform.Translate (-dragDirection, transform);
								}
							}
						}
					} else {
			...
			if (dragDampingStart > 0 && !buttonLeftPressed) {
				...
				dragging = true;
				_currentCamera.transform.Translate (-dragDirection * t, transform);
			}
